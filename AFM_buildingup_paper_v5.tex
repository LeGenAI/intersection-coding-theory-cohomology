...
code of length 12 are self-dual, that is,
...
Kim~\cite{Kim2001} introduced ``building-up construction'' which proved that all binary self-dual codes can be constructed in this way. This construction starts from a binary self-dual code of length $2n$ and then adds a vector of length $2n$ of odd weight to construct a binary self-dual code of length $2n+2$. Kim--Lee~\cite{KimLee2009},~\cite{KimLee} generalized this method to construct self-dual codes over finite fields $\mathbb F_q$, where $q$ is even or $q \equiv 1 \pmod{4}$.
 In practice, this is a principal method for constructing examples from a
 split bilinear space. However, the formula for the new generator rows is
 often presented in terms of an auxiliary vector whose inner product with
 itself is $-1$. One of the central points of the present paper is that the
 corresponding correction coordinates are naturally organized by an
 isotropic line in a hyperbolic plane, and that the forward construction
 admits a precise row-wise and family-wise characterization in those terms.
...
\(r=\dim(\mathcal C\cap U_c)\).  After a scalar-coordinate permutation, one may
...
\]
For a linear code, the minimum distance equals the minimum Hamming weight of
its nonzero codewords.  For a linear code
$\mathcal C\subseteq\F_q^n$, we write
\[
\mathcal C^\perp :=
\{v \in \F_q^n : \langle v,c\rangle =0\text{ for all }c\in\mathcal C\}.
\]
For subspaces \(\mathcal A,\mathcal B\subseteq V\), put
\(\mathcal A\cap\mathcal B:=\{v\in V:v\in\mathcal A\text{ and }
v\in\mathcal B\}\).  For a matrix \(G\) over \(K\),
\(\langle G\rangle_K\) denotes the code spanned by its rows.

\begin{defn}[self-dual code]\label{def:sd}
{\rm
A linear code $\mathcal C\subseteq\F_q^n$ is \emph{self-dual} if
$\mathcal C=\mathcal C^\perp$~\cite{Pless1965}.
}
\end{defn}
...
\(\mathcal C\subseteq\F_q^n\) is self-dual if and only if it is totally isotropic
and \(2\dim\mathcal C=n\). Thus a self-dual code is precisely a Lagrangian
...
Let $\mathcal C\subseteq\F_q^{2k}$ be generated by a matrix
$G=[I_k\mid A]$ with $A\in M_k(\F_q)$. Then \(\mathcal C\) is self-dual if and only if
\[
AA^T = -I_k.
\]
\end{prop}
...
{\rm
Writing codewords as row vectors, two codes
$\mathcal C_1,\mathcal C_2\subseteq\F_q^n$ are
\emph{permutation equivalent} if there is a permutation \(\sigma\in S_n\)
such that
\[
 \mathcal C_2=\mathcal C_1P_\sigma,
\]
where \(P_\sigma\) is the permutation matrix associated with \(\sigma\).
}
\end{defn}

In the Lean notation below, \texttt{permuteVec~\(\sigma\)~u} denotes
the row-vector product \(uP_\sigma\).  Coordinate permutations are Euclidean
isometries:
\[
\langle uP_\sigma,vP_\sigma\rangle=\langle u,v\rangle.
\]
...
\begin{proof}
For (i), since \(q\) is odd, \(-1\ne1\). Thus \(c^4=1\) and
\(c^2\ne1\), so \(c\) has order \(4\).

For (ii), take \(A=cI_k\). Then
\(AA^T=c^2I_k=-I_k\).
...
Let $G_0$ be a generator matrix of a binary self-dual code
$\mathcal C_0$ of length $2n$, whose rows are $g_1, \dots, g_n$. Let
$\bf x$ be a binary vector in $\mathbb F_2^{2n}$ satisfying
${\bf x}\cdot{\bf x}=1$. Suppose that $y_i={\bf x}\cdot g_i$ for
$1\le i\le n$. Then the following matrix
\[
\begingroup
...
\endgroup
\]
generates a binary self-dual code $\mathcal C$ of length $2n+2$.
\end{theorem}
...
\begin{theorem} (\cite[Theorem 2]{Kim2001})  \label{thm-binary-build-2}
 Any binary self-dual code $\mathcal C$ of length $2n$ with minimum distance $d>2$ is obtained from some self-dual code $\mathcal C_0$ of length
...
\(\iota_E\) above and put \(\mathcal C=\iota_E(L)\). Suppose that the coordinate
...
\(\mathcal C'=\langle G\rangle_{\mathbb F_2}\). Then:
\begin{enumerate}
\item \(L\) is Lagrangian for \(B_{\rm coh}\), while \(\mathcal C\) and the boxed
representative \(\langle G'\rangle_{\mathbb F_2}\) are Euclidean self-dual.
\item In the displayed Euclidean coordinates,
\[
E_2=\langle(1,0),(0,1)\rangle=\mathbb F_2^2,
\qquad
...
hence \(\mathcal C'\) is Euclidean self-dual of length \(2n-2\).
...
\begin{proof}
Let \(P_\sigma\) denote the selected permutation matrix and define
\(\eta(u)=\iota_E(u)P_\sigma\). Since right multiplication by
\(P_\sigma\) and \(\iota_E\) are form isometries,
\[
\eta(u)\cdot\eta(v)=B_{\rm coh}(u,v)
\qquad(u,v\in W).
\]
Thus \(\mathcal C_\sigma:=\eta(L)=\langle G'\rangle_{\mathbb F_2}\) is Euclidean
self-dual and has dimension \(n\). Since \(\iota_E\) is also a form
isometry,
\[
\mathcal C=\mathcal C^\perp,
\qquad
\mathcal C_\sigma=\mathcal C_\sigma^\perp.
\]
This proves (1).

The first two standard coordinate vectors \((1,0)\) and \((0,1)\) span the
plane \(E_2\) with Gram matrix \(I_2\). In particular,
\[
...
It remains to verify the inverse relation in (5).  The first row of the
Kim matrix constructed from \(x\) and \(G\) is \((1,0,x)=r_0\), and its
row corresponding to \(g_i\) is
\[
 \bigl(x\mathbin{\cdot}g_i,x\mathbin{\cdot}g_i,g_i\bigr)
   =(y_i,y_i,g_i)=r_i.
\]
Hence the constructed matrix is equal to \(G'\) row by row.  Conversely,
deleting \(r_0\) and the first two coordinates sends every remaining row
\(r_i\) to \(g_i\), and therefore recovers \(G\).  Thus reduction followed
by building-up gives \(G'\) exactly.  Since
\(\langle G'\rangle_{\mathbb F_2}=\mathcal C P_\sigma\), right multiplication by
\(P_\sigma^{-1}\) shows that the reconstructed code is permutation
equivalent to \(\mathcal C\), as
claimed.
\end{proof}
...
\(\eta(u)=\iota_E(u)P_\sigma\), so \texttt{hcode} identifies the actual
...
Let $\mathbb F_q$ be a finite field with $q$ elements. Assume that $q \equiv 1 \pmod{4}$. Let $c$ be in $\mathbb F_q$ such that $c^2 = -1$. Let $G_0$ be a generator matrix of a Euclidean self-dual code $\mathcal C_0$ over $\mathbb F_q$ of length $2n$, whose rows are $g_1, \dots, g_n$. Let $\bf x$ be a vector in $\mathbb F_q^{2n}$ satisfying ${\bf x} \cdot {\bf x} = -1$. Suppose that $y_i ={\bf x} \cdot g_i$ for $1 \le i \le n$. Then the following matrix
\[
\begingroup
\setlength{\arraycolsep}{6pt}
\renewcommand{\arraystretch}{1.2}
...
\endgroup
\]
generates a self-dual code $\mathcal C$ over $\mathbb F_q$ of length $2n+2$.
\end{theorem}
...
Let $\mathbb F_q$ be a finite field with $q$ elements. Assume that $q \equiv 1 \pmod{4}$. Any Euclidean self-dual code $\mathcal C$ over $\mathbb F_q$ of length $2n$ with minimum distance $d >2$ is obtained from some Euclidean self-dual code $\mathcal C_0$ over $\mathbb F_q$ of length $2n-2$ (up to permutation equivalence) by the construction method in Theorem~\ref{thm-q-ary-build-1}.
\end{theorem}
...
\begin{enumerate}
\item Let \(E=K^2\) denote the first coordinate plane in the displayed
decomposition of \(W\).
...
\[
L = \Bigl\langle(1,0,x),\;
(-\langle x,g_i\rangle,\,-c\langle x,g_i\rangle,\,g_i)\;:\;1 \le i \le n-1\Bigr\rangle_K,
\]
...
\begin{proof}
The decomposition \(W=E\perp W_0\), with \(E=K^2\), is
...
Let \(\mathcal C\subset K^{2n}\) be a self-dual code, and fix \(c\in K^\times\) with \(c^2=-1\). Assume that, after coordinate permutations preserving the \(2\)-block decomposition and elementary row operations, \(\mathcal C\) admits a generator matrix whose last row is
\[
...
\end{enumerate}
Then this generator matrix is automatically a split boxed matrix of Theorem~\ref{thm:split-boxed-form}: the self-duality of \(\mathcal C\) forces
\[
u_i^2+v_i^2=-1,
\qquad
u_i+c v_i=-c a_i,
...
c(b_{ij}+b_{ji})+\ell_i\cdot \ell_j=0,
\]
and hence \(\mathcal C\) is generated by a matrix of the displayed split boxed type.
\end{theorem}
...
\begin{example}[A fixed rank-two pairing over \(\GF{5}\)]
\label{ex:rank-two-gf5}
\rm
Take \(c=2\), so \(c^2=-1\), and let \(\mathcal C\) be generated by
\[
G_0=\begin{pmatrix}
1&3&3&2&1&1&1&3\\
3&4&2&1&4&4&3&2\\
...
\end{pmatrix}.
\]
All calculations in this example are in \(\GF{5}\). One checks that
\(G_0G_0^{\mathsf T}=0\), while the minor formed by the first four columns
has determinant \(4\). Hence \(\mathcal C\) has dimension \(4\) and is self-dual.

Pair adjacent coordinates and let \(G_0^\sigma\) be obtained by ordering the
four two-coordinate blocks as \((2,4,1,3)\). Writing
\(a_i=Q_{ii}\), the diagonal formula in
\eqref{eq:universal-rank-r-box} gives \(a_1=4\) and \(a_2=3\). We
substitute these values and \(c=2\), but retain the displayed diagonal
formula in the second coordinate: the two blocks are
\((4,2\cdot4+1)\) and \((3,2\cdot3+1)\). The invertible row transformation
\[
T=\begin{pmatrix}
4&1&0&0\\
2&1&2&0\\
...
\left[\begin{array}{c!{\vrule width 1.2pt}c!{\vrule width 0.6pt}cc}
(4,2\cdot4+1)&\multicolumn{1}{c!{\vrule width 0.6pt}}{2(1,2)}&(2,1)&(3,3)\\[-1pt]
\noalign{\hrule height 1.2pt}
4(1,2)&(3,2\cdot3+1)&(2,3)&(0,2)\\[-1pt]
\noalign{\hrule height 0.6pt}
(1,2)&4(1,2)&(1,2)&(1,2)\\
4(1,2)&2(1,2)&(1,2)&2(1,2)
\end{array}\right]}.
\]
This is \eqref{eq:universal-rank-r-box} with
\[
\ell_1=((2,1),(3,3)),\qquad
\ell_2=((2,3),(0,2)),\qquad
Q_{12}=2,\quad Q_{21}=4,
\]
and
\[
D=\begin{pmatrix}1&1\\1&2\end{pmatrix},\qquad \det D=1.
\]
Modulo five the diagonal pairs are \((4,4)\) and \((3,2)\), respectively.
For a block \((u,v)\), recall that \(\partial_2(u,v)=v-2u\).  Hence
\[
\begin{aligned}
\partial_2\ell_1
  &=\begin{pmatrix}1-2\cdot2\\3-2\cdot3\end{pmatrix}
...
\end{aligned}
\]
Similarly, \(\partial_2\ell_2=(4,2)^{\mathsf T}\) and
\(-D\partial_2\ell_2=(4,2)^{\mathsf T}\).
The off-diagonal relation
\eqref{eq:universal-rank-r-relation} is satisfied. Applying the blockwise
defect map gives
\[
\partial G_{2,2}=
\begin{pmatrix}
...
so
\[
r=\dim(\mathcal C\cap U_2)=4-\rank(\partial G_{2,2})=2.
\]
The block permutation preserves \(U_2\); consequently, whole-block
permutations and row operations cannot reduce \(r\) for this fixed oriented
pairing.

Deleting pivot rows together with their corresponding block columns gives
\[
G_{1,2}=
{\arrayrulewidth=1.2pt
\left[\begin{array}{c!{\vrule width 1.2pt}cc}
(3,2\cdot3+1)&(2,3)&(0,2)\\[-1pt]
\noalign{\hrule height 1.2pt}
4(1,2)&(1,2)&(1,2)\\
2(1,2)&(1,2)&2(1,2)
...
\end{pmatrix}.
\]
Thus Theorem~\ref{thm:qary-universal-rank-r} gives the exact self-dual chain
\[
[8,4]\longrightarrow[6,3]\longrightarrow[4,2],
\]
with the same terminal matrix \(D\) at every level and
\(\langle G_{0,2}\rangle_K=K(1,2)\oplus K(1,2)\).
\end{example}
...
For a linear code \(\mathcal C\subset K^{2n}\), the following are equivalent:
\begin{enumerate}
\item \(\mathcal C=\mathcal C^\perp\);
\item there are a permutation \(\pi\in S_{2n}\) and
...
such that
\[
\mathcal C P_\pi=\langle G_1(c;b,\ell)\rangle_K.
\]
\end{enumerate}
In these coordinates,
\[
\dim\bigl((\mathcal C P_\pi)\cap U_c\bigr)=1.
\]
...
\[
\dim\bigl((\mathcal C P_{\pi_0})\cap U_c\bigr)=1.
\]

Let \(s\) span \((\mathcal C P_{\pi_0})\cap U_c\).  Some block of \(s\) is nonzero;
after permuting whole blocks and rescaling \(s\), take its last block to be
\((1,c)\).  The restriction to \(\mathcal C P_{\pi_0}\) of
\[
...
The fixed-pairing rank in Example~\ref{ex:rank-two-gf5} does not preclude a
scalar-coordinate permutation that changes the pairing. The next example
applies Theorem~\ref{thm:qary-universal-rank-one} to the same code and, at
the same time, shows that rank one does not force the diagonal blocks to be
\((0,1)\).

\begin{example}[The rank-one permutation of
Example~\ref{ex:rank-two-gf5}]
\label{ex:rank-one-permutation-gf5}
\rm
Return to the original generator \(G_0\) in
Example~\ref{ex:rank-two-gf5}. Let \(\tau\) interchange only its first two
scalar coordinates, so the ordered pairs become
\((2,1),(3,4),(5,6),(7,8)\). With
\[
T_1=\begin{pmatrix}
0&2&2&0\\
2&2&0&0\\
1&2&0&0\\
1&4&1&1
\end{pmatrix},\qquad \det T_1=4,
\]
exact row reduction gives
\[
G_{3,1}=T_1G_0^\tau=
{\arrayrulewidth=1.2pt
\left[\begin{array}{c!{\vrule width 1.2pt}c!{\vrule width 1.2pt}c!{\vrule width 0.6pt}c}
(1,2\cdot1+1)&\multicolumn{1}{c}{0(1,2)}&2(1,2)&(4,3)\\[-1pt]
\noalign{\hrule height 1.2pt}
4(1,2)&(0,2\cdot0+1)&0(1,2)&(3,0)\\[-1pt]
\cline{2-4}
1(1,2)&2(1,2)&(4,2\cdot4+1)&(2,2)\\[-1pt]
\noalign{\hrule height 0.6pt}
0(1,2)&1(1,2)&2(1,2)&(1,2)
\end{array}\right]}.
\]
Thus its three diagonal blocks display the substituted pairs
\((1,2\cdot1+1)\), \((0,2\cdot0+1)\), and
\((4,2\cdot4+1)\), corresponding to
\((a_1,a_2,a_3)=(1,0,4)\) with \(c=2\). Its defect matrix is
\[
\partial_2G_{3,1}=
\begin{pmatrix}
1&0&0&0\\
...
\end{pmatrix},
\qquad
\dim\bigl((\mathcal C P_\tau)\cap U_2\bigr)=4-3=1.
\]
More precisely, \(G_{3,1}\) is the matrix \(G_1(c;b,\ell)\) of
Theorem~\ref{thm:qary-universal-rank-one} with
\[
\ell_1=(4,3),\qquad \ell_2=(3,0),\qquad \ell_3=(2,2),
\]
\[
(a_1,a_2,a_3)=(1,0,4),\qquad
(b_{12},b_{13},b_{21},b_{23},b_{31},b_{32})=(0,2,4,0,1,2).
\]
Indeed, \((\delta_1,\delta_2,\delta_3)=(0,4,3)\), so the last row has
coefficients \(-(\delta_1,\delta_2,\delta_3)=(0,1,2)\), and all three
relations \eqref{eq:universal-rank-one-relation} hold. Thus the permutation
does not merely lower a numerical defect rank: it produces the literal
universal rank-one generator with \(D=(1)\). The nonzero diagonal
coefficients \(a_1\) and \(a_3\) show explicitly why
Theorem~\ref{thm:qary-universal-rank-one} does not imply the zero-diagonal
specialization of Theorem~\ref{thm:split-boxed-form}.
\end{example}
...
\rm
Let \(\mathcal C_{20}^{(13)}\) be the pure double-circulant self-dual code generated
by
\[
...
permutation, one obtains a self-dual parent \(\mathcal C_{18}^{(13)}\) with parameters
...
\left\langle\mathcal B_5
 \bigl(G_{18}^{(13)},x_{18}^{(13)}\bigr)\right\rangle_{\GF{13}}
 =\mathcal C_{20}^{(13)}.
\]
...
\(\mathcal C_{18}^{(13)}\longleftrightarrow\mathcal C_{20}^{(13)}\).}
