...
def build_outputs():
    ...
    for symbol, code, relation in [
            (r"C_{16}^{(2)}", c16, "base parent"),
            (r"C_{18}^{(2)}", c18, r"$\mathcal B_1(G_{16},x_{16})$"),
            (r"C_{20}^{(2)}", c20, r"$\mathcal B_1(G_{18},x_{18})$"),
            (r"C_{22}^{(2)}", c22, r"$\mathcal B_1(G_{20},x_{20})$"),
            (r"\mathcal G_{24}", c24, r"$\mathcal B_1(G_{22},x_{22})$")]:
        n, k, d = code["parameters"]
        evidence = f"\\href{{{REPOSITORY}}}{{${symbol}$}}"
        catalogue_rows.append(
            f"{evidence} & 2 & $[{n},{k},{d}];\\,"
            f"\\mathbf{{{code['a_d']:,}}}$ & "
            f"$({d};\\,{code['a_d']:,})$ & Harada--Munemasa DB; {relation} \\\\"
        )
