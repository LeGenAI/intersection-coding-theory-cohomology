...
def build_outputs():
    ...
    for level, name, code, relation in [
            ...
            (1, r"$C_{18}$", c18, r"Kim$(C_{16},x_{16})$"),
            (2, r"$C_{20}$", c20, r"Kim$(C_{18},x_{18})$"),
            (3, r"$C_{22}$", c22, r"Kim$(C_{20},x_{20})$"),
            (4, r"$\mathcal G_{24}$", c24, r"Kim$(C_{22},x_{22})$")]:
    ...
    catalogue_rows = []
    for artifact, code in [("BIN-16", c16), ("BIN-18", c18),
                           ("BIN-20", c20), ("BIN-22", c22),
                           ("BIN-24", c24)]:
        n, k, d = code["parameters"]
        evidence = ("\\href{" + REPOSITORY + "}{\\texttt{" + artifact + "}}")
        catalogue_rows.append(
            f"{evidence} & 2 & $[{n},{k},{d}];\\,{code['a_d']:,}$ & "
            f"$({d};\\,{code['a_d']:,})$ & Huffman--Pless \\\\"
        )
