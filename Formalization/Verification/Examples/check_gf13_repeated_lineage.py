...
def build_outputs():
    ...
    g22 = circulant_source(source["source"]["first_circulant_row"], p)
    c22 = certify_matrix(g22, p, source["source"]["parameters"])
    c22["a_d"] = source["source"]["a_d"]
    c22["minimum_weight_verified_by_magma"] = True
    ...
    certificate = {
        "schema_version": 1,
        "artifact_id": "GF13-REPEATED-18-22",
        "source_sha256": hashlib.sha256(source_bytes).hexdigest(),
        "interpretation": (
            "Two exact two-coordinate reductions from the published GF(13) "
            "[22,11,10] code and literal inverse Kim--Lee presentations."
        ),
        "levels_ascending": [c18, c20, c22],
        "steps_ascending": list(reversed(steps)),
        ...
        "magma_audit": {
            ...
            "first_reduction_ordered_pairs": 462,
            "second_reduction_ordered_pairs": 380,
            "second_reduction_zero_weight_eight_loss_pairs": 338
        }
    }
    results = {
    ...
    catalogue_rows = []
    for artifact, code in [("GF13-R18", c18), ("GF13-R20", c20), ("GF13-R22", c22)]:
        n, k, d = code["parameters"]
        benchmark = benchmark_by_length[n]
        link = f"\\href{{{REPOSITORY}}}{{\\texttt{{{artifact}}}}}"
    ...
    tex += f"\\newcommand{{\\GFThirteenTwentyCorrection}}{{{tuple_tex(x20)}}}\n"
    tex += ("\\newcommand{\\GFThirteenRepeatedCertificate}{\\href{" + REPOSITORY +
            "}{\\texttt{GF13-REPEATED-18-22}}}\n")
    return {
...
def main():
    ...
    print("PASS GF(13) repeated lineage: [18,9,7] -> [20,10,8] -> [22,11,10]")
