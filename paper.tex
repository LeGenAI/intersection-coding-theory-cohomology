...
The applications below use the boxed forms developed above to give
explicit self-dual codes over \(\GF{5}\) and \(\GF{13}\).

\subsection{Applications}\label{sec:applications}
\input{Formalization/Verification/Examples/applications_data.tex}
...
\cite{LeonPlessSloane,KimChoi2021}. All residues below are represented by
integers between \(0\) and \(p-1\), where \(p=5\) or \(13\).
The parameters, matrices, and witness codewords for the first four rows of
Table~\ref{tab:application-distances} are generated from
\texttt{applications.json} in the accompanying artifact.
The standard-library Python script \texttt{check\_applications.py}
checks the boxed relations, full row rank, and \(GG^{\mathsf T}=0\).
It also enumerates all \(p^k\) coefficient vectors for each \(k\times2k\)
matrix \(G\), recording
\[
A_j=\#\{a\in\mathbb F_p^k:\operatorname{wt}(aG)=j\}.
\]
For the final row, \texttt{check\_large\_applications.py} verifies all
\(\binom{14}{7}=3432\) seven-column information sets and the exact universal
normalization. Table~\ref{tab:application-distances} gives the resulting
minimum distance \(d\) and \(A_d\); in each row,
\(A_0=1\) and \(A_j=0\) for \(0<j<d\).
The complete distributions, which also satisfy the MacWilliams identity,
are recorded in \texttt{applications\_results.json}; the large normalization
and parent data are recorded in \texttt{large\_applications\_results.json}.
These distance computations are reproducible exact computations, not Lean
certificates. The Gram, normalization, deletion, and self-duality identities
for the large example are checked separately by the Lean kernel.
...
\toprule
Proposition & \(p\) & Parameters & Nonzero vectors & \(d\) & \(A_d\)\\
\midrule
\AppDistanceTable
\AppThirteenFourteenDistanceRow
\bottomrule
\end{tabular}
\caption{Exact minimum-distance data for the application matrices. The first four rows use complete coefficient enumeration; the final row uses the MDS minor criterion.}
\label{tab:application-distances}
\end{table}

\begin{prop}[An optimal self-dual \texorpdfstring{$[6,3,4]$}{[6,3,4]} code over \texorpdfstring{$\GF{5}$}{GF(5)}]
\label{ex:split-boxed-63}
\rm
Let \(K=\GF{5}\), \(c=2\), and choose
\AppFiveSixParameters
The corresponding two-coordinate block matrix is
\[
\widetilde G_3=\AppFiveSixBlocks.
\]
It generates an optimal self-dual \([6,3,4]\) code.
\end{prop}

\begin{proof}
The displayed parameters satisfy the three boxed relations, so
Corollary~\ref{thm:split-boxed-form} gives self-duality. In scalar coordinates,
\[
G_3=\AppFiveSixMatrix.
\]
Its second row has weight \(4\), while the enumeration summarized
in Table~\ref{tab:application-distances} gives \(A_1=A_2=A_3=0\).
Thus \(d=4\). The Singleton bound gives \(d\le6-3+1=4\) for every
linear \([6,3]\) code, proving optimality.

Deleting the first block row and block column gives
\[
\widetilde G_2=
\begin{pmatrix}
01 & 02\\
...
\end{pmatrix},
\]
which generates the corresponding self-dual \([4,2,2]\) code.
\end{proof}

\begin{prop}[An optimal self-dual \texorpdfstring{$[8,4,4]$}{[8,4,4]} code over \texorpdfstring{$\GF{5}$}{GF(5)}]
\label{ex:split-boxed-84}
\rm
Let \(K=\GF{5}\), \(c=2\), and choose
\AppFiveEightParameters
The corresponding two-coordinate block matrix is
\[
\widetilde G_4=\AppFiveEightBlocks.
\]
It generates an optimal self-dual \([8,4,4]\) code.
\end{prop}

\begin{proof}
Corollary~\ref{thm:split-boxed-form} gives self-duality, and the scalar matrix is
\[
G_4=\AppFiveEightMatrix.
\]
The coefficient vector \(\AppFiveEightWitness\) gives the word
\[
\AppFiveEightWord,
\]
of weight \(4\). Table~\ref{tab:application-distances} gives
\(A_1=A_2=A_3=0\), so \(d=4\).
Any linear \([8,4]\) code with larger minimum distance would be an
\([8,4,5]\) MDS code by the Singleton bound. Ball's bound
\cite{Ball2012}, applied with \(k=4\le p=5\), excludes this: its generator
columns would form a set of eight vectors every four of which are a basis,
whereas such a set has at most \(p+1=6\) elements. Hence the code is optimal.
\end{proof}

Deleting the first block row and block column from \(\widetilde G_4\)
recovers exactly \(\widetilde G_3\).

\begin{prop}[An optimal self-dual \texorpdfstring{$[8,4,5]$}{[8,4,5]} code over \texorpdfstring{$\GF{13}$}{GF(13)}]
\label{ex:split-boxed-134}
\rm
Let \(K=\GF{13}\), \(c=5\), and choose
\AppThirteenEightParameters
The corresponding generator is
\[
G=\AppThirteenEightMatrix.
\]
It generates an optimal self-dual \([8,4,5]\) code.
\end{prop}

\begin{proof}
The boxed relations give self-duality by Corollary~\ref{thm:split-boxed-form}.
The coefficient vector \(\AppThirteenEightWitness\) produces
\[
\AppThirteenEightWord,
\]
of weight \(5\). Table~\ref{tab:application-distances} has \(A_j=0\)
for \(1\le j<5\), so \(d=5\). This meets the Singleton bound
\(d\le8-4+1=5\), proving optimality among all linear \([8,4]\) codes.
\end{proof}

\begin{remark}
\label{rem:optimal-gf13-example}
\rm
The preceding code is MDS; thus the split boxed construction can attain
the Singleton bound over \(\GF{13}\).
\end{remark}

\begin{prop}[An optimal self-dual \texorpdfstring{$[10,5,6]$}{[10,5,6]} code over \texorpdfstring{$\GF{13}$}{GF(13)}]
\label{ex:split-boxed-135}
\rm
Let \(K=\GF{13}\), \(c=5\), and choose
\AppThirteenTenParameters
The corresponding generator is
\[
G_{10}=\AppThirteenTenMatrix.
\]
It generates an optimal self-dual \([10,5,6]\) code.
\end{prop}

\begin{proof}
The boxed relations give self-duality by Corollary~\ref{thm:split-boxed-form}.
The coefficient vector \(\AppThirteenTenWitness\) produces
\[
\AppThirteenTenWord,
\]
of weight \(6\). Table~\ref{tab:application-distances} has \(A_j=0\)
for \(1\le j<6\), so \(d=6\). The Singleton bound is
\(d\le10-5+1=6\), proving optimality among all linear \([10,5]\) codes.
\end{proof}

\begin{prop}[A universal rank-one realization of a self-dual MDS \texorpdfstring{$[14,7,8]$}{[14,7,8]} code over \texorpdfstring{$\GF{13}$}{GF(13)}]
