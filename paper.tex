...
\toprule
Code & \(p\) & Our \([n,k,d];A_d\) & Published \((d_{\rm best};A_d)\) & Reference\\
\midrule
\GolayCatalogueRows
...
\bottomrule
\end{tabular}
\caption{Certified self-dual codes and published distance benchmarks.}
...
The complete normalized generator is
\[
\resizebox{\textwidth}{!}{$
\widetilde G_{24}^{(2)}=\BinaryLargestRepeatedMatrix .
$}
\]
The vertical rules delimit the twelve two-coordinate blocks; they are not
...
one-based, and \GFThirteenRepeatedCertificate\ links to the matrices and their
verification record.
...
\[
\resizebox{\textwidth}{!}{$
\widetilde G_{22}^{(13)}=\GFThirteenLargestRepeatedMatrix .
$}
\]
...
For every upward edge in Table~\ref{tab:repeated-lineages}, the corresponding
...
\bottomrule
\end{tabular}
\caption{Repeated boxed lineages over \(\GF{2}\), \(\GF{5}\), and \(\GF{13}\).}
