...
\item As concrete applications, we exhibit the largest certified repeated
boxed presentations over \(\GF{2}\), \(\GF{5}\), and \(\GF{13}\), ending at
self-dual codes with parameters \([24,12,8]\), \([24,12,9]\), and
\([22,11,10]\), respectively.  For a known pure double-circulant
...
Section~\ref{sec:applications} gives the certified applications over
\(\GF{2}\), \(\GF{5}\), and \(\GF{13}\).
...
\subsection{Verified application catalogue}
...
\input{Formalization/Verification/Examples/gf5_repeated_top_data.tex}
...
The same two-coordinate border displays the largest certified code over each
...
orders recorded in the certificates, the three largest presentations are
\begin{equation}
\begin{aligned}
\widetilde G_{24}^{(2)}
...
\widetilde G_{22}^{(13)}
  &=\mathcal B_5(G_{20}^{(13)},x_{20}^{(13)}).
\end{aligned}
\label{eq:three-largest-repeated-boxes}
\end{equation}
Each equality is an exact generator identity after the certified row
...
self-dual \([22,11,8]\) parent \(C_{22}^{(5)}\).  For the certified parent
generator \(G_{22}^{(5)}\),
\[
...
132 distance-nine-loss-free ordered pairs among all \(24\cdot23=552\).

Over \(\GF{13}\), let \(C_{22}^{(13)}\) be the pure double-circulant
...
For the certified parent generators \(G_{18}^{(13)}\) and
...
\GolayLineageRows
\addlinespace
\multicolumn{5}{c}{\(\GF{5}\) lineage}\\
\GFFiveRepeatedLineageRows
\addlinespace
\multicolumn{5}{c}{\(\GF{13}\) lineage}\\
\GFThirteenRepeatedLineageRows
\bottomrule
\end{tabular}
\caption{The verified lineages supporting the three largest repeated boxed
presentations.  The checkers verify
rank, self-orthogonality, the recorded weight distributions and their
MacWilliams identities, literal reconstruction of every displayed repeated
box, and equality with the child row space.  The full matrices are
in certificates \GolayLineageCertificate,
\GFFiveRepeatedCertificate, and \GFThirteenRepeatedCertificate.}
...
We have studied self-dual codes over finite fields $\F_q$ with $q\equiv 1 \pmod 4$ from three complementary viewpoints: the classical building-up construction, the hyperbolic geometry forced by the split condition $-1\in (K^\times)^2$, and a Lean~4 formalization of the resulting algebraic statements.  Section~\ref{sec:applications} exhibits the largest certified repeated boxed presentations over all three fields considered here: the binary Golay endpoint \([24,12,8]\), a self-dual \([24,12,9]\) code over \(\GF{5}\), and a self-dual \([22,11,10]\) code over \(\GF{13}\).  Each is displayed through the same two-coordinate border and an exact certified parent.  The universal form also gives an explicit rank-one realization and exact two-coordinate parent of a self-dual MDS \([14,7,8]\) code over \(\GF{13}\).  The algebraic results are formalized in Lean~4.
