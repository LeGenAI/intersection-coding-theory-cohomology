...
\item As concrete applications, we obtain explicit optimal self-dual codes \([6,3,4]\) and \([8,4,4]\) over \(\GF{5}\), explicit optimal self-dual codes \([8,4,5]\) and \([10,5,6]\) over \(\GF{13}\), and an explicit self-dual \([12,6,6]\) code over \(\GF{13}\), all arising from the split boxed construction (Section~\ref{sec:applications}).
...
The applications below use Corollary~\ref{thm:split-boxed-form} to give
...
The parameters, matrices, and witness codewords are generated from the
single data file \texttt{applications.json} in the accompanying artifact.
Its standard-library Python script \texttt{check\_applications.py}
...
\]
Table~\ref{tab:application-distances} gives the resulting minimum distance
\(d=\min\{j>0:A_j\ne0\}\) and \(A_d\).
In every row, \(A_0=1\) and \(A_j=0\) for \(0<j<d\).
The complete distributions, which also satisfy the MacWilliams identity,
are recorded in \texttt{applications\_results.json}.
These are reproducible exhaustive computations, not Lean certificates.
...
\bottomrule
\end{tabular}
\caption{Exhaustive minimum-distance computations for the application matrices.}
...
\begin{prop}[An explicit self-dual \texorpdfstring{$[12,6,6]$}{[12,6,6]} code over \texorpdfstring{$\GF{13}$}{GF(13)}]
\label{ex:split-boxed-136}
\rm
Let \(K=\GF{13}\), \(c=5\), and choose
\AppThirteenTwelveParameters
The corresponding generator is
\[
G_{12}=\AppThirteenTwelveMatrix.
\]
It generates a self-dual \([12,6,6]\) code.
\end{prop}

\begin{proof}
The boxed relations give self-duality by Corollary~\ref{thm:split-boxed-form}.
The coefficient vector \(\AppThirteenTwelveWitness\) produces
\[
\AppThirteenTwelveWord,
\]
of weight \(6\). The enumeration of all \(13^6-1\) nonzero coefficient
vectors gives \(A_j=0\) for \(1\le j<6\), as recorded in
Table~\ref{tab:application-distances}. Hence \(d=6\).
\end{proof}

\begin{remark}
\label{rem:minimum-distance-scope}
\rm
The \([8,4,5]\) and \([10,5,6]\) examples over \(\GF{13}\) are optimal, whereas the \([12,6,6]\) example establishes the stated minimum distance without an optimality claim. These computations do not assert that the construction always maximizes minimum distance.
\end{remark}

\section{Conclusion}
\label{sec:conclusion}

We have studied self-dual codes over finite fields $\F_q$ with $q\equiv 1 \pmod 4$ from three complementary viewpoints: the classical building-up construction, the hyperbolic geometry forced by the split condition $-1\in (K^\times)^2$, and a Lean~4 formalization of the resulting algebraic statements. On the computational side, this yields explicit small-length applications over \(\GF{5}\) and \(\GF{13}\), including several optimal examples produced directly from the split boxed construction. The algebraic results are formalized in Lean~4.
