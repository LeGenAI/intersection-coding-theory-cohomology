...
isotropic line governing the split construction, and we prove that every
nonzero-length Euclidean self-dual code over \(\F_q\) has, after a
scalar-coordinate permutation, an explicit rank-one boxed representative.
Applications include a recursive optimal pair over \(\GF{5}\) and an exact
repeated boxed realization
\([18,9,8]_{13}\leftrightarrow[20,10,10]_{13}\), in which the rank-one
...
the Euclidean plane is hyperbolic. In the binary theorem of
...
\item After a scalar-coordinate permutation, every nonzero-length
Euclidean self-dual code over \(\F_q\), \(q\equiv1\pmod4\), has the explicit
universal rank-one normal form of
Theorem~\ref{thm:qary-universal-rank-one}.

\item The applications consist of a recursive quinary pair and one complete
repeated boxed realization
\([18,9,8]_{13}\leftrightarrow[20,10,10]_{13}\), displayed as an exact
parent--child generator (Section~\ref{sec:applications}).
...
greater than one.  Theorem~\ref{thm:qary-universal-rank-one} obtains rank one
only after a scalar-coordinate permutation.
\end{remark}

\subsection{Universal normal form}\label{sec:universal-normal-form}

The preceding results distinguish two questions.  For a fixed pairing of
the \(2n\) coordinates, the intersection of a code with the product of the
isotropic lines \(K(1,c)\) can have dimension greater than one.  A
permutation of the scalar coordinates changes that pairing while preserving
the Euclidean form and every Hamming weight.  With this freedom the terminal
part can always be reduced to one row.
...
\noindent
The formal proof separates the universal pairing from the forward
self-duality calculation:
\begin{lstlisting}[caption={Lean goals for Theorem~\ref{thm:qary-universal-rank-one}.}]
...
forms.  The first is a short recursive calculation over \(\GF{5}\).  The
second is a single nontrivial
...
\subsection{A recursive quinary pair}
...
\begin{prop}[A split boxed child \texorpdfstring{$[8,4,4]$}{[8,4,4]} over \texorpdfstring{$\GF{5}$}{GF(5)}]
...
propositions constitute one recursive calculation,
\[
...
\begin{prop}[A repeated boxed realization over
\texorpdfstring{\(\GF{13}\)}{GF(13)}]
\label{prop:gf13-repeated-realization}
\rm
...
statements, and then removed the adapted-presentation hypothesis by the
universal rank-one normal form.  Thus the same two-coordinate mechanism
