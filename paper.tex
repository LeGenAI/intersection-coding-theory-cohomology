...
The applications below separate three kinds of computational output.  Table~1
compares reproducible examples with the corresponding published benchmarks,
a separate binary example records an exact two-level descent and ascent, and a
proposition is reserved for the universal realization that supplies a
distinguished parent and a fixed-seed continuation problem.
...
Each artifact identifier links to its generator, complete weight distribution,
and verification metadata.  The table makes no claim that a listed benchmark
value of \(A_d\) is extremal.}
\label{tab:application-distances}
\end{table}

The full matrices formerly displayed as separate propositions are retained in
the linked certificates.  The numerical verifier checks their boxed
relations, rank, Gram matrix, complete weight distribution, and MacWilliams
identity.  Thus moving them to Table~\ref{tab:application-distances} changes
only the exposition, not the available evidence.  The finite enumerations are
external exact computations rather than Lean proofs.

\paragraph{A length-20-centered Golay descent and ascent.}
...
are recorded separately in Table~\ref{tab:golay-lineage}.  In particular, the
...
\bottomrule
\end{tabular}
\caption{The verified length-20-centered Golay lineage.  Complete enumeration checks
rank, self-orthogonality, distance, weight distribution, and the MacWilliams
identity.  At each upward step the checker also verifies literal reconstruction
of the displayed Kim matrix and equality of its row space with the child code;
the full matrices are in certificate \GolayLineageCertificate.}
\label{tab:golay-lineage}
\end{table}
