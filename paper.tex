...
by a nonsingular $r\times r$ matrix $D$; orthogonality forces the opposite
block to be \(-DQ^{\mathsf T}\), leaving only one Gram relation among the
independent coefficients.  Simultaneous pivot-row and block-column
...
of \(C\cap U_c\). Write \(G(c;P,H,Q,D)\) for this matrix and set
\[
...
\hline
-(DQ^{\mathsf T})_{\bullet1}(1,c) &
-(DQ^{\mathsf T})_{\bullet2}(1,c) & \cdots &
-(DQ^{\mathsf T})_{\bullet k}(1,c)
...
and \(-(DQ^{\mathsf T})_{\bullet j}(1,c)\) denotes the column of blocks
\(-(DQ^{\mathsf T})_{sj}(1,c)\), \(1\leq s\leq r\). Here
\(P\in M_k(K)\), \(H,Q\in M_{k\times r}(K)\), and the terminal block is specified by
\(D\in M_r(K)\), which satisfies
\begin{equation}
\label{eq:universal-rank-core}
\det D\ne0.
\end{equation}
The remaining coefficients satisfy exactly
\begin{equation}
 I_k+c(P+P^{\mathsf T})
...
Reading the first coordinates and the differences \(\beta_j\) of the pivot
rows produces \(P,H,Q\), while the terminal first coordinates of the
\(S_s\) produce \(D\).  Let \(a_{sj}\) be the first coordinate of the
...
\[
a_{si}+\sum_{t=1}^r Q_{it}D_{st}=0.
\]
Thus \(a_{si}=-(DQ^{\mathsf T})_{si}\), which gives
...
Finally, pivot--pivot orthogonality is exactly
\eqref{eq:universal-rank-pp}, while products between the \(S_s\) vanish
identically.  This proves the existence direction.  Conversely, the chosen
lower-left coefficients make pivot--terminal products vanish identically,
and \eqref{eq:universal-rank-pp} gives the remaining products.  Applying the
...
\(P'=P_{I,I}\), \(H'=H_{I,*}\), and \(Q'=Q_{I,*}\).
The entries of the retained matrix give \eqref{eq:rank-boxed-restriction}
literally because the retained lower-left block is
\(-D(Q')^{\mathsf T}\).  Restricting \eqref{eq:universal-rank-pp} to rows
and columns in \(I\) gives
\[
I_l+c(P'+(P')^{\mathsf T})
...
\(\gamma_1=(P_{21},\ldots,P_{k1},
-(DQ^{\mathsf T})_{11},\ldots,-(DQ^{\mathsf T})_{r1})^{\mathsf T}\).
...
the terminal blocks \((-DQ^{\mathsf T},D)\) by
\((-Q^{\mathsf T},I_r)\) without changing the code.  We retain \(D\) only
...
Let \(P,H,Q,D\) satisfy the hypotheses of the converse in
...
\begin{proof}
Since \(K\) has odd characteristic, \(2\) is invertible.  The forced
master-by-pivot block is
\[
-D(Q^+)^{\mathsf T}
=\begin{pmatrix}-Dq^{\mathsf T}&-DQ^{\mathsf T}\end{pmatrix},
\]
so its old columns are unchanged.  For the Gram relation, use its equivalent
symmetric-part identity.
...
\(P,H,Q,D\) entry by entry. The resulting coefficients again satisfy
...
theorem determined_repeated_step_kim_lee_iff {k r : Nat} (c : K)
    ...
    Iff
      (Exists fun x => And (dot x x = -1)
        (rowSpace (readSuccessor (extendedRows c P H Q
            (forcedMasterCoefficients Q D) D h q u)) =
...
-DQ^{\mathsf T}=\begin{pmatrix}1&4\\4&2\end{pmatrix},\qquad \det D=1.
\]
...
diagonal. The displayed coefficients satisfy \eqref{eq:universal-rank-pp}.
...
\(H_{i1}=Q_{i1}=1\); the forced lower-left coefficient is also (1).
With \(c=1\), the terminal pivot block is then
...
P_{ij}=b_{ij}\ (i\ne j),\qquad
H_{i1}=u_i,\qquad Q_{i1}=v_i-cu_i.
\end{equation}
...
\(G(c;P,H,Q,(1))\), with the final row and block column placed
last, precisely when its forced lower coefficient
\(-Q_i=cu_i-v_i\) equals \(a_i\).  Because \(c\ne0\) and
\(c^2=-1\), this equality is equivalent to
\[
...
gives a generator \(G(c;P,H,Q,D)\), after a permutation of the
...
The verified catalogue then compares the resulting codes with published
benchmarks, and the final subsection displays the largest repeated
parent--child realizations.

\input{sections/application_universal_gf13.tex}

\subsection{Certified application catalogue}
...
\(G(c;P,H,Q,D)\), rather than left as an arbitrary generator.  Within the
parent, the teal rows and paired columns display
\(P_{ij}(1,c)+\delta_{ij}(0,1)\); the violet terminal rows and columns display
the invertible core \(D_{st}(1,c)\), with the remaining violet block forced to
be \(-DQ^T(1,c)\).  Blue is the newly adjoined row and orange is the newly
adjoined coordinate pair.  The outer thick rules mark the building-up border,
whereas the inner thick rules separate the \(P\)-part from the \(D\)-part of
the universal parent.
For the binary endpoint the displayed parent has
\(\BinaryLargestParentParameters\).  A permutation within each binary pivot
pair orients every diagonal pair as \((0,1)\); equivalently, \(P_{ii}=0\) for
all \(i\), exactly as in the normal form of Theorem~\ref{thm:binary-cz-kim}.
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{$
...
\mathcal G_{24}\) with universal parent.}
...
$}
\caption{Repeated realization over \(\GF{5}\):
\(C_{22}^{(5)}\longleftrightarrow C_{24}^{(5)}\), with universal parent.}
...
$}
\caption{Repeated realization over \(\GF{13}\):
\(C_{20}^{(13)}\longleftrightarrow C_{22}^{(13)}\), with universal parent.}
