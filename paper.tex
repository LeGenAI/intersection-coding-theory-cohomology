...
with a publicly accessible benchmark at the same length.  A bold value in our
column attains the smallest \(A_d\) among codes of the displayed best distance
in a complete online classification, or is forced by the MDS weight
enumerator.  We make no minimum claim at unclassified lengths.  This
distinction matters over \(\GF{13}\): the four published length-12
...
\toprule
Code & \(p\) & Our \([n,k,d];A_d\) & Public benchmark \((d_{\rm best};A_d)\) & Reference\\
\midrule
\GolayCatalogueRows
