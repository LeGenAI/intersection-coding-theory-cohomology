...
a universal rank-$r$ boxed normal form.  Its lower-right block is specified
by a nonsingular $r\times r$ matrix $D$, subject to the displayed Gram
relations.  Simultaneous pivot-row and block-column
...
\item As concrete applications, we obtain explicit optimal self-dual codes \([6,3,4]\) and \([8,4,4]\) over \(\GF{5}\) and MDS self-dual codes over \(\GF{13}\).  For a known pure double-circulant \([14,7,8]\) code, we give an exact universal rank-one generator, its \([12,6,6]\) two-coordinate parent, and the unique projective correction that can restore distance~8 over this parent (Section~\ref{sec:applications}).
...
The recorded inputs were unchanged throughout the run. The four separate
numerical example certificates and the external minimum-distance
computations also passed, but are not included in the 123-goal count.

The paper is organized as follows. Section~\ref{sec:background} fixes notation and the split quadratic background. Section~\ref{sec:building-up} first collects the building-up tools, then presents the universal rank-\(r\) boxed form, its binary rank-one refinement, and the normalized rank-one forms over odd fields, followed by concrete applications over \(\GF{5}\) and \(\GF{13}\). Section~\ref{sec:conclusion} closes with the comparison viewpoint and the formalization perspective.
...
over odd fields in Section~\ref{sec:odd-rank-one}.  The last subsection gives
...
To describe the pivot coefficients, write each two-coordinate block as
\((\alpha,c\alpha+\beta)\), where \(c^2=-1\). In the first \(k\) block
columns, the pivot rows are normalized so that their \(\beta\)-coordinates
form the identity matrix. Writing \(P_{ij}\) for the corresponding
\(\alpha\)-coordinate therefore gives the block
\((P_{ij},cP_{ij}+\delta_{ij})\). Thus \(P\) is a matrix of coefficients,
not a coordinate permutation; it need not be symmetric, triangular, or
have zero diagonal.
...
of \(C\cap U_c\). Write \(G(c;P,H,Q,A,D)\) for this matrix and set
\[
L_i=\bigl((H_{it},\,cH_{it}+Q_{it})\bigr)_{t=1}^r.
\]
Then
\begin{equation}
...
\left[
\begin{array}{cccc|c}
(P_{11},cP_{11}+1) & P_{12}(1,c) & \cdots & P_{1k}(1,c) & L_1\\
P_{21}(1,c) & (P_{22},cP_{22}+1) & \cdots & P_{2k}(1,c) & L_2\\
\vdots & \vdots & \ddots & \vdots & \vdots\\
P_{k1}(1,c) & P_{k2}(1,c) & \cdots & (P_{kk},cP_{kk}+1) & L_k\\
\hline
A_{\bullet1}(1,c) & A_{\bullet2}(1,c) & \cdots & A_{\bullet k}(1,c)
  & \bigl(D_{st}(1,c)\bigr)_{r\times r}
\end{array}
\right],
\endgroup
\end{equation}
where the upper-left block has entries \((P_{ij},cP_{ij}+\delta_{ij})\)
and \(A_{\bullet j}(1,c)\) denotes the column of blocks
\(A_{sj}(1,c)\), \(1\leq s\leq r\). Here
\(P\in M_k(K)\), \(H,Q\in M_{k\times r}(K)\),
\(A\in M_{r\times k}(K)\), and the terminal block is specified by
\(D\in M_r(K)\), which satisfies
\begin{equation}
...
\begin{align}
 A+DQ^{\mathsf T}&=0,
 \label{eq:universal-rank-pm}\\
 I_k+c(P+P^{\mathsf T})
 +c(HQ^{\mathsf T}+QH^{\mathsf T})+QQ^{\mathsf T}&=0.
 \label{eq:universal-rank-pp}
\end{align}
Since \(c^{-1}=-c\), the second relation is equivalently
\[
P+P^{\mathsf T}=c(I_k+QQ^{\mathsf T})
 -(HQ^{\mathsf T}+QH^{\mathsf T}).
\]
For fixed \(H,Q\), it determines the symmetric part of \(P\), while its
skew-symmetric part remains arbitrary.
Conversely, for every \(k,r\geq0\), every matrix
\(G(c;P,H,Q,A,D)\) satisfying
\eqref{eq:universal-rank-core}--\eqref{eq:universal-rank-pp} has
...
G_I=G(c;P_{I,I},H_{I,*},Q_{I,*},A_{*,I},D),
\end{equation}
...
Reading the first coordinates and the differences \(\beta_j\) of these rows produces matrices
\(P,H,Q,A,D\) and gives \eqref{eq:universal-rank-box}.  It remains to prove
that the terminal matrix \(D\) is nonsingular.  For \(S\in\ker\partial\), write
...
Finally, the block identity
\[
(\alpha,c\alpha+\beta)\cdot(\gamma,c\gamma+\delta)
=c(\alpha\delta+\beta\gamma)+\beta\delta
\]
shows that orthogonality between the rows \(g_i\) and \(S_s\) is exactly
\eqref{eq:universal-rank-pm}, while pivot--pivot orthogonality is exactly
\eqref{eq:universal-rank-pp}; products between the \(S_s\) vanish identically.
This proves the existence direction.  Conversely, these two relations give
pairwise orthogonality.  Applying the linear functional \((s,t)\mapsto t-cs\) blockwise to the first
...
For the recursive assertion, put
\(P'=P_{I,I}\), \(H'=H_{I,*}\), \(Q'=Q_{I,*}\), and \(A'=A_{*,I}\).
The entries of the retained matrix give \eqref{eq:rank-boxed-restriction}
literally.  Restricting \eqref{eq:universal-rank-pm} to columns in \(I\), and
\eqref{eq:universal-rank-pp} to rows and columns in \(I\), gives
\[
A'+D(Q')^{\mathsf T}=0,\qquad
I_l+c(P'+(P')^{\mathsf T})
 +c(H'(Q')^{\mathsf T}+Q'(H')^{\mathsf T})+Q'(Q')^{\mathsf T}=0.
\]
The products involving \(H'\) and \(Q'\) still sum over all \(r\) terminal
columns.  Since \(D\) is unchanged, the converse already proved applies.
...
\]
Here \(\rho_1=(P_{12}(1,c),\ldots,P_{1k}(1,c),L_1)\) and
\(\gamma_1=(P_{21},\ldots,P_{k1},A_{11},\ldots,A_{r1})^{\mathsf T}\).
...
The integer \(r\) records \(C\cap U_c\) for the fixed coordinate pairing.
The entries of \(D\), however, depend on the chosen basis of \(C\cap U_c\):
multiplication on the left by \(\operatorname{diag}(I_k,D^{-1})\) replaces
\((A,D)\) by \((D^{-1}A,I_r)\) without changing the code.  Universality
...
Let \(P,H,Q,A,D\) satisfy the hypotheses of the converse in
Theorem~\ref{thm:universal-rank-boxed}. Choose any row vectors
\(h,q\in K^r\) and \(u\in K^k\), and put
\[
\begin{aligned}
p&=\frac{c}{2}(1+qq^{\mathsf T})-hq^{\mathsf T},\\
v&=cQq^{\mathsf T}-Hq^{\mathsf T}-Qh^{\mathsf T}-u^{\mathsf T}.
\end{aligned}
\]
Then the coefficients
\[
P^+=\begin{pmatrix}p&u\\v&P\end{pmatrix},\qquad
H^+=\begin{pmatrix}h\\H\end{pmatrix},\qquad
Q^+=\begin{pmatrix}q\\Q\end{pmatrix},\qquad
A^+=\begin{pmatrix}-Dq^{\mathsf T}&A\end{pmatrix}
\]
and the same \(D\) define a matrix \(G_{k+1,r}\) whose row space
is self-dual.
...
\begin{proof}
Since \(K\) has odd characteristic, \(2\) is invertible. First,
\[
A^++D(Q^+)^{\mathsf T}
=\begin{pmatrix}0&A+DQ^{\mathsf T}\end{pmatrix}=0.
\]
For the other Gram relation, use its equivalent symmetric-part identity.
The old lower-right block is unchanged, while the new diagonal and
off-diagonal blocks satisfy
\[
2p=c(1+qq^{\mathsf T})-2hq^{\mathsf T},\qquad
v+u^{\mathsf T}=cQq^{\mathsf T}-Hq^{\mathsf T}-Qh^{\mathsf T}.
\]
Consequently,
\[
P^++(P^+)^{\mathsf T}
=c\bigl(I_{k+1}+Q^+(Q^+)^{\mathsf T}\bigr)
 -\bigl(H^+(Q^+)^{\mathsf T}+Q^+(H^+)^{\mathsf T}\bigr).
\]
Theorem~\ref{thm:universal-rank-boxed} now gives independence and
self-duality. Restricting to the old rows and paired columns recovers
\(P,H,Q,A,D\) entry by entry. The resulting coefficients again satisfy
all the hypotheses, so the construction can be repeated. Finally, the defect
...
\(m=k+r\). Put
\[
\gamma=\begin{pmatrix}v\\-Dq^{\mathsf T}\end{pmatrix},\qquad
\rho=\bigl((u_j,cu_j)_{j=1}^k,
                  (h_t,ch_t+q_t)_{t=1}^r\bigr).
\]
...
\[
\neg\bigl(q=0\ \text{and}\ u^{\mathsf T}=-Qh^{\mathsf T}\bigr).
\]
More precisely, if \(\gamma_s\ne0\), set
\begin{equation}\label{eq:repeated-kim-normalization}
\lambda=\frac{c-p}{\gamma_s},\qquad
...
If \(\gamma=0\), instead,
\[
p=c/2,\qquad \rho\in\operatorname{row}(G),\qquad
...
If \(\gamma_s\ne0\), then
\[
p+\lambda\gamma_s=c,\qquad
cp+1+\lambda c\gamma_s=c^2+1=0.
\]
...
Consequently \(\rho\cdot\rho=0\), and \(1+2cp=0\) gives \(p=c/2\).
...
Finally, \(D\) is invertible, so \(Dq^{\mathsf T}=0\) forces \(q=0\);
with \(q=0\), the equality \(v=0\) is precisely
\(u^{\mathsf T}=-Qh^{\mathsf T}\).
\end{proof}
...
(p,cp+1),\qquad p=c-\sum_{j=1}^k a_j\gamma_j.
\]
...
\]
Set \(u_j=\rho_{2j-1}\); in each terminal block write
\(\rho=(h_t,ch_t+q_t)\). Orthogonality of \(b'_0\) to the
terminal rows and then to the pivot rows gives, respectively,
\[
\gamma_{k+s}=-\sum_t q_tD_{st},\qquad
\gamma_i=c\sum_t q_tQ_{it}
 -\sum_t(h_tQ_{it}+q_tH_{it})-u_i.
\]
Finally, isotropy of \(b'_0\) gives
\[
0=1+2cp+2c\sum_t h_tq_t+\sum_t q_t^2,
\qquad
p=\frac c2\left(1+\sum_t q_t^2\right)-\sum_t h_tq_t.
\]
...
\begin{lstlisting}[caption={Exact fixed-parent criterion in Lemma~\ref{lem:repeated-kim-lee}.}]
theorem repeated_step_kim_lee_iff {k r : Nat} (c : K)
    (P : Fin k -> Fin k -> K) (H Q : Fin k -> Fin r -> K)
    (A : Fin r -> Fin k -> K) (D : Fin r -> Fin r -> K)
    (hc : c ^ 2 = (-1 : K)) (h2 : Not ((2 : K) = 0))
    (hD : RankBoxCoreFullRank D)
    (hpm : PivotMasterRelations Q A D)
    (hpp : PivotGramRelations c P H Q)
    (h q : Fin r -> K) (u : Fin k -> K) :
    Iff
      (Exists fun x => And (dot x x = -1)
        (rowSpace (readSuccessor (extendedRows c P H Q A D h q u)) =
          rowSpace (buildRows x c (flattenRows (rankBoxedRows c P H Q A D)))))
      (Not (And (q = 0)
        (forall i, u i = -(Finset.univ.sum fun t => Q i t * h t))))
\end{lstlisting}
Here the paired coordinates are listed consecutively by \texttt{flattenRows},
and \texttt{readSuccessor} places the new pair first. The independent
formal statements also include the explicit matrix equality in
\eqref{eq:repeated-kim-normalization}, the direct-sum classification,
and the reverse first-row operation in
...
\[
P=\begin{pmatrix}4&2\\4&3\end{pmatrix},\qquad
H=\begin{pmatrix}2&3\\2&0\end{pmatrix},\qquad
Q=\begin{pmatrix}2&2\\4&2\end{pmatrix},
\]
\[
A=\begin{pmatrix}1&4\\4&2\end{pmatrix},\qquad
D=\begin{pmatrix}1&1\\1&2\end{pmatrix},\qquad \det D=1.
\]
For example, the four upper-left blocks have first coordinates
\(4,2,4,3\), which are exactly the entries of \(P\). Its diagonal blocks
are \((4,2\cdot4+1)=(4,4)\) and \((3,2\cdot3+1)=(3,2)\),
whereas its off-diagonal blocks are \(2(1,2)\) and \(4(1,2)\).
In particular, \(P\) is neither symmetric nor triangular and has nonzero
diagonal. The displayed coefficients satisfy
\eqref{eq:universal-rank-pm}--\eqref{eq:universal-rank-pp}.
...
\[
\begin{array}{c|ccc}
\text{step}&h&q&u\\ \hline
G_{2,2}\longrightarrow G_{3,2}&(1,2)&(2,1)&(1,2)\\
G_{3,2}\longrightarrow G_{4,2}&(2,4)&(1,3)&(1,2,4)
\end{array}
\]
with each new pivot prepended. Both steps give \(p=2\). The final matrix is
\[
G_{4,2}=
\left[\begin{array}{cc|cc|cc}
(2,0)&(1,2)&(2,4)&(4,3)&(2,0)&(4,1)\\
(4,3)&(2,0)&(1,2)&(2,4)&(1,4)&(2,0)\\ \hline
(1,2)&(3,1)&(4,4)&(2,4)&(2,1)&(3,3)\\
(3,1)&(1,2)&(4,3)&(3,2)&(2,3)&(0,2)\\ \hline
...
gives \(G_{3,2}\). Thus, with arrows denoting building-up and its
...
G_{4,2}\ ([12,6]).
\]
All three row spaces are Euclidean self-dual, and all have \(r=2\) and
the same terminal matrix \(D\).

These two extensions also give the literal Kim--Lee construction, not
just matrices satisfying the Gram equations. Multiplying the first row
of each successor by \(3=2^{-1}\) changes its first block to \((1,0)\).
The corresponding tail vectors are
\[
x_1=(3,1,1,2,3,2,1,0),\qquad
x_2=(3,1,1,2,2,4,1,0,2,3),
\]
and \(x_1\cdot x_1=x_2\cdot x_2=4=-1\).
For each parent row \(g_i\), the newly prepended pair is
\((-y_i,3y_i)\), where \(y_i=x_j\cdot g_i\).
Thus Theorem~\ref{thm-q-ary-build-1} applies with its square root of
\(-1\) chosen as \(3=-2\); the boxed matrices themselves use \(c=2\).
Only a generator-row scaling is involved, so neither the Euclidean form
nor the coordinate pairing is changed. Lemma~\ref{lem:rank-boxed-building-up},
beyond these two numerical steps, ensures further extensions with the same \(D\).
Since these two steps have \(p=c\), their row scalings are the case
\(\lambda=0\) of Lemma~\ref{lem:repeated-kim-lee};
Lemma~\ref{lem:kim-lee-repeated} supplies the converse for every
Kim--Lee extension of either parent.
\end{example}

\subsection{Binary rank-one refinement}
\label{sec:binary-refinement}

The same matrix formula also applies over \(\F_2\).  For positive length,
Theorem~\ref{thm:binary-universal-rank-one} gives a scalar-coordinate
permutation with \(r=1\), \(D=(1)\), zero pivot diagonal, and
\(H_{i1}=Q_{i1}=A_{1i}=1\).  With \(c=1\), the terminal pivot block is then
\((1,1+1)=(1,0)\), so \eqref{eq:universal-rank-box} is the literal binary box.
...
division by two. In a normalized binary CZ step, \(c=1\), \(p=0\),
...
\begin{theorem}[Cohomological transport and binary boxed reduction]\label{thm:binary-cz-kim}
...
Since \(\iota_E\) preserves the two forms, it transports orthogonal
complements, so \(C=\iota_E(L)\) is Euclidean self-dual.
...
To recover the following rank-one form, set \(k=n-1\), \(r=1\), \(D=(1)\)
\emph{and} \(P_{ii}=0\).  The latter is an additional diagonal normalization,
not a consequence of \(r=1\).  The exact coefficient correspondence is
\begin{equation}
\label{eq:rank-one-split-dictionary}
P_{ij}=b_{ij}\ (i\ne j),\qquad
H_{i1}=u_i,\qquad Q_{i1}=v_i-cu_i,\qquad A_{1i}=a_i.
\end{equation}
...
\begin{proof}
Use the parameters in \eqref{eq:rank-one-split-dictionary}, with \(P_{ii}=0\).
Since
\[
\bigl(u_i,cu_i+(v_i-cu_i)\bigr)=(u_i,v_i)=\ell_i,
\]
the matrix in the corollary is literally
\(G(c;P,H,Q,A,(1))\), with the final row and block column placed
last.  Write \(Q_i=v_i-cu_i\).  The equation in
\eqref{eq:universal-rank-pm} becomes \(a_i+Q_i=0\); because \(c\ne0\) and
\(c^2=-1\), this is equivalent to
\[
c(a_i+Q_i)=ca_i+u_i+cv_i=0,
\]
which is \eqref{eq:split-boxed-last-row}.  The diagonal entry of
\eqref{eq:universal-rank-pp} is
\[
1+2cu_iQ_i+Q_i^2=1+u_i^2+v_i^2,
\]
so its vanishing is exactly \eqref{eq:split-boxed-norm}.  For \(i\ne j\),
the corresponding entry is
\[
c(b_{ij}+b_{ji})+c(u_iQ_j+Q_i u_j)+Q_iQ_j
=c(b_{ij}+b_{ji})+\ell_i\cdot\ell_j.
\]
Symmetry reduces these equations to \(i<j\), giving precisely
\eqref{eq:split-boxed-offdiag}.  Thus the three hypotheses are equivalent
to the two universal Gram relations, and \(\det(1)=1\).
...
gives a generator \(G(c;P,H,Q,A,D)\), after a permutation of the
two-coordinate blocks, with
\[
r=\dim(C\cap U_c),\qquad k=n-r,\qquad \det D\ne0.
\]
When \(r=1\), scaling the terminal row normalizes \(D\) to \((1)\).
The further conditions \(P_{ii}=0\) are additional assumptions, not
consequences of \(r=1\). Under these conditions, the substitution
\eqref{eq:rank-one-split-dictionary} reduces the Gram relations to
...
The applications below separate reproducible catalogue entries, repeated
parent--child lineages, and the universal realization used in the main
argument.

\subsection{Verified application catalogue}\label{sec:applications}
\input{Formalization/Verification/Examples/application_catalog_data.tex}
\input{Formalization/Verification/Examples/large_applications_data.tex}
\input{Formalization/Verification/Examples/golay_lineage_data.tex}
\input{Formalization/Verification/Examples/gf13_repeated_lineage_data.tex}
...
with a published benchmark at the same length.  The benchmark \(A_d\) is the
coefficient of the cited representative code, not an extremal quantity.  This
distinction matters at length~12 over \(\GF{13}\), where inequivalent optimal
codes have different weight enumerators.  For MDS rows, by contrast,
...
\begin{tabular}{@{}lc>{\centering\arraybackslash}p{75pt}
  >{\centering\arraybackslash}p{95pt}>{\raggedright\arraybackslash}p{100pt}@{}}
\toprule
Artifact & \(p\) & Our \([n,k,d];A_d\) & Published \((d_{\rm best};A_d)\) & Source\\
\midrule
\GolayCatalogueRows
\ApplicationCatalogueRows
\GFThirteenRepeatedCatalogueRows
\bottomrule
\end{tabular}
\caption{Verified application catalogue compared with published benchmarks.
Each artifact identifier links to its generator and verification metadata;
complete weight distributions are included whenever they were computed.  The
table makes no claim that a listed benchmark value of \(A_d\) is extremal.}
\label{tab:application-distances}
\end{table}

The full matrices formerly displayed as separate propositions are retained in
the linked certificates.  The numerical verifiers check their relevant boxed
or repeated relations, rank, Gram matrix, and the recorded weight data; every
complete distribution is also checked against the MacWilliams identity.  Thus
moving them to Table~\ref{tab:application-distances} changes only the
exposition, not the available evidence.  The finite enumerations are external
exact computations rather than Lean proofs.

\subsection{Repeated lineages centered at length~20}
...
identity.
The parameters and the number \(A_d\) of minimum-weight words at each level
are recorded in Table~\ref{tab:repeated-lineages}.  In particular, the
...
There is an odd-characteristic analogue over \(\GF{13}\) using the same
repeated mechanism.  Let \(C_{22}^{(13)}\) be the pure double-circulant
self-dual \([22,11,10]\) code whose circulant first row is
\[
(7,7,8,3,0,10,1,9,7,11,10),
\]
as in~\cite{BetsumiyaEtAl}.  Exact two-coordinate reduction at the ordered
pair \((1,16)\) gives a self-dual \([20,10,8]\) code
\(C_{20}^{(13)}\), and reduction of this same code at \((1,2)\) gives a
self-dual \([18,9,7]\) code \(C_{18}^{(13)}\).  The coordinate indices are
one-based and refer to the matrices in certificate
\GFThirteenRepeatedCertificate.

For a self-dual parent matrix \(G\) with rows \(g_i\), put
\[
\mathcal K_5(G,x)=
\begin{pmatrix}
1&0&x\\
...
-y_m&-5y_m&g_m
\end{pmatrix},
\qquad y_i=x\mathbin{\cdot}g_i.
\]
This is the normalization of Theorem~\ref{thm:qary-building-up} with
\(c=5\).  For the certified parent generators \(G_{18}^{(13)}\) and
\(G_{20}^{(13)}\), the correction vectors are
\[
\begin{aligned}
x_{18}^{(13)}&=\GFThirteenEighteenCorrection,\\
x_{20}^{(13)}&=\GFThirteenTwentyCorrection.
\end{aligned}
\]
They satisfy \(x_m^{(13)}\mathbin{\cdot}x_m^{(13)}=-1\) and the exact
row-space identities
\[
\begin{aligned}
\operatorname{rowspan}\mathcal K_5
  (G_{18}^{(13)},x_{18}^{(13)})&=C_{20}^{(13)},\\
\operatorname{rowspan}\mathcal K_5
  (G_{20}^{(13)},x_{20}^{(13)})&=C_{22}^{(13)}.
\end{aligned}
\]
Consequently these are not separately selected examples but one
bidirectional lineage
\[
C_{18}^{(13)}\ \longleftrightarrow\ C_{20}^{(13)}
\ \longleftrightarrow\ C_{22}^{(13)}.
\]
The checker verifies both literal repeated matrices, equality with the
corresponding child row spaces, and the complete weight distributions at
lengths 18 and 20.  The Magma audit examines all \(22\cdot21=462\) ordered
pairs at the first reduction and all \(20\cdot19=380\) at the second.
...
\begin{tabular}{@{}clccl@{}}
\toprule
Level & Code & Parameters & \(A_d\) & Exact upward relation\\
\midrule
\multicolumn{5}{c}{Binary lineage}\\
\GolayLineageRows
\addlinespace
\multicolumn{5}{c}{\(\GF{13}\) lineage}\\
\GFThirteenRepeatedLineageRows
\bottomrule
\end{tabular}
\caption{The two verified lineages centered at length~20.  The checkers verify
rank, self-orthogonality, the recorded weight distributions and their
MacWilliams identities, literal reconstruction of every displayed Kim or
Kim--Lee matrix, and equality with the child row space.  The full matrices are
in certificates \GolayLineageCertificate\ and
\GFThirteenRepeatedCertificate.}
\label{tab:repeated-lineages}
\end{table}

\subsection{A universal rank-one MDS realization over \texorpdfstring{$\GF{13}$}{GF(13)}}
...
row-equivalent universal generator is obtained from the coefficient matrix
\begin{equation}
P=\AppThirteenFourteenP
\label{eq:gf13-14-coefficient-p}
\end{equation}
and
\[
...
\qquad
A=\AppThirteenFourteenA=-Q^{\mathsf T},
\qquad D=(1).
\]
Let \(\delta_{ij}\) denote the Kronecker delta: \(\delta_{ij}=1\) when
\(i=j\), and \(\delta_{ij}=0\) otherwise.  It records the identity readout
in the second coordinate of the \(j\)-th pivot block; explicitly,
\[
(P_{ij},5P_{ij}+\delta_{ij})=
\begin{cases}
(P_{ii},5P_{ii}+1),&i=j,\\
(P_{ij},5P_{ij}),&i\ne j.
\end{cases}
\]
In the notation of Theorem~\ref{thm:universal-rank-boxed}, this gives
\begin{equation}
\widehat G_{14}=G(5;P,H,Q,A,D)
=\left[
\begin{array}{c|c}
\bigl((P_{ij},5P_{ij}+\delta_{ij})\bigr)_{1\le i,j\le6}
  & \bigl((H_i,5H_i+Q_i)\bigr)_{1\le i\le6}\\
\hline
\bigl(A_j(1,5)\bigr)_{1\le j\le6} & (1,5)
\end{array}
\right].
\label{eq:gf13-14-structured-generator}
\end{equation}
Thus the six pivot rows have exactly the pattern of
Theorem~\ref{thm:universal-rank-boxed}, while every block of the terminal row
is a multiple of \((1,5)\). It has \(k=6\), \(r=1\), and \(D=(1)\).
Deleting its first block row and
first block column gives a self-dual \([12,6,6]\) parent. For this displayed
parent, the projective correction classes satisfying the necessary
weight-six/seven lifting condition form the singleton
\[
\Lambda_8=\{\AppThirteenFourteenSurvivor\}.
\]
The correction column of \(\widehat G_{14}\) is
\(\gamma=\AppThirteenFourteenGamma\), whose projective class is the displayed
element of \(\Lambda_8\); rebuilding recovers \(\widehat G_{14}\).
\end{prop}
...
\(\partial(s,t)=t-5s\) blockwise makes the rank-one structure visible:
\[
...
The universal Gram relations hold for the extracted \(P,H,Q,A,D\), so
Theorem~\ref{thm:universal-rank-boxed} gives self-duality both before and
after the stated simultaneous row-and-block deletion.
...
\]
and the lower six rows of \eqref{eq:gf13-14-structured-generator}, after deleting their
first block, are literally the parent generator. Reattaching that block and
the first row therefore recovers \(\widehat G_{14}\).
\end{proof}

This exact parent--child pair suggests a fixed-seed measure of the
construction.  Let \(C_0=\langle G_{14}\rangle\), and for \(m\ge0\) define
\[
\begin{aligned}
\delta_m(G_{14})=\max\{d(C_m):\;&C_{i+1}\text{ is obtained from }C_i\\
 &\text{by one non-direct-sum step of
 Lemma~\ref{lem:rank-boxed-building-up}}\}.
\end{aligned}
\]
The computational question is whether \(\delta_m(G_{14})\) reaches the
best possible minimum distance of a self-dual code of length \(14+2m\).
The correction-space search first rejects projective correction classes that
are orthogonal to a low-weight coefficient line of the fixed parent; exact
minimum distance is computed only for the survivors.  Any continuation from
\texttt{GF13-14} will be recorded in a separate lineage certificate, so that
Table~\ref{tab:application-distances} remains a comparison with the published
history rather than mixing benchmark and ancestry data.

\section{Conclusion}
\label{sec:conclusion}

We have studied self-dual codes over finite fields $\F_q$ with $q\equiv 1 \pmod 4$ from three complementary viewpoints: the classical building-up construction, the hyperbolic geometry forced by the split condition $-1\in (K^\times)^2$, and a Lean~4 formalization of the resulting algebraic statements. On the computational side, the universal form gives an explicit rank-one realization and exact two-coordinate parent of a self-dual MDS \([14,7,8]\) code over \(\GF{13}\).  Repetitive examples are collected in a linked certificate table, leaving the universal realization and its fixed-seed continuation problem as the principal odd-characteristic application.  The separate Golay computation exhibits four successive exact reductions and inverse Kim steps through the optimal binary chain \([16,8,4]\to[18,9,4]\to[20,10,4]\to[22,11,6]\to[24,12,8]\). The algebraic results are formalized in Lean~4.

In the binary case, the universal rank-one normalization proves intrinsically that every Euclidean self-dual code has the literal Chinburg--Zhang boxed representative under a scalar-coordinate permutation.  Theorem~\ref{thm:binary-cz-kim} then transports the Chinburg--Zhang cohomological pairing through a specified orthonormal-basis isometry and compares the resulting Euclidean boxed matrix with Kim's building-up extension.  The top-down deletion is a coordinate reduction through a non-alternating first coordinate plane, not deletion of a hyperbolic pair.
...
integer for the fixed coordinate pairing is \(r=\dim(C\cap U_c)\), and the
